...
class Installer{
	...
	static async install(_appRootPath, _skipElectronVersionCheck = false){
		let promise = await new Promise((resolve, reject) => {
			if(!Utils.isGlasscordDownloaded())
				Utils.downloadGlasscordAsar().then((result) => {
					if(result) resolve(this._installRoutine(_appRootPath, _skipElectronVersionCheck));
					else resolve({error: "Unable to download Glasscord"});
				});
			else
				resolve(this._installRoutine(_appRootPath, _skipElectronVersionCheck));
		});
		return promise;
	}

	static async uninstall(_appRootPath){
		return Promise.resolve(this._uninstallRoutine(_appRootPath));
	}

	static _installRoutine(_appRootPath, _skipElectronVersionCheck = false){
		const app = this._checkApp(_appRootPath, _skipElectronVersionCheck);
	...
	static _uninstallRoutine(_appRootPath){
		const app = this._checkApp(_appRootPath);
		if(app.error) return app;
	...
	static _checkApp(_appRootPath, _skipElectronVersionCheck = false){
		const resourcesPath = Utils.easyGetResourcesPath(_appRootPath);
		...
		if(!_skipElectronVersionCheck)
			electronVersion = Utils.getElectronVersion(Utils.getMainExecutableCandidates(_appRootPath))
