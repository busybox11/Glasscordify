...
class Installer{
		...
		let promise = await new Promise((resolve, reject) => {
			if(!Utils.isGlasscordDownloaded())
				Utils.downloadGlasscordAsar().then((result) => {
					if(result) resolve(this._installRoutine(_appExecutablePath, _skipElectronVersionCheck));
					else resolve({error: "Unable to download Glasscord"});
				});
			else
				resolve(this._installRoutine(_appExecutablePath, _skipElectronVersionCheck));
		});
		return promise;
	}
	...
	static _installRoutine(_appExecutablePath, _skipElectronVersionCheck = false){
		const app = this._checkApp(_appExecutablePath, _skipElectronVersionCheck);
	...
	static _uninstallRoutine(_appExecutablePath){
		const app = this._checkApp(_appExecutablePath, true);
		if(app.error) return app;
	...
	static _checkApp(_appExecutablePath, _skipElectronVersionCheck = false){
