...
class Utils{
		...
		let promise = await new Promise((resolve, reject) => {
			...
			this.httpsGet("https://api.github.com/repos/AryToNeX/Glasscord/releases/latest", {headers: {"user-agent": "glasscordify"}}, result => {
				...
				this.httpsGet(url, {headers: {"user-agent": "glasscordify"}}, file => {
					...
					// Save data
					fs.ensureDirSync(path.resolve(this.getConfigPath(), "glasscord", "_bin"));
					fs.writeFileSync(path.resolve(this.getConfigPath(), "glasscord", "_bin", "glasscord._asar"), file.data);
					fs.renameSync(path.resolve(this.getConfigPath(), "glasscord", "_bin", "glasscord._asar"), path.resolve(this.getConfigPath(), "glasscord", "_bin", "glasscord.asar"))
					return resolve(true);
				});
			});
		});
	...
	static revertMainFile(_mainFile){
		if(isMainFilePatched(_mainFile)){
			const oldFile = fs.readFileSync(_mainFile).toString();
	...
	static commitAsarExtraction(_resourcesPath){
		fs.renameSync(path.resolve(_resourcesPath, "app.gcextracted"), path.resolve(_resourcesPath, "app"));
		return true;
	}
	...
	static getAbsoluteExecutableFile(_probableLink){ // TODO: Testing and fixing
		...
		switch(ext){
			...
			case ".desktop": // Linux desktop file
				const file = fs.readFileSync(_probableLink, {encoding: 'utf-8'});
				let exec = file.match(/^Exec=([^\s]*)/m);
				if(exec.length === 0) return Promise.resolve(undefined);

				console.log(exec);
				if(path.isAbsolute(exec[1]))
				...
				for(let _path of paths)
					if(fs.existsSync(path.resolve(_path, exec[1])))
						return Promise.resolve(path.resolve(_path, exec[1]));
