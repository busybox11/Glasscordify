...
class Utils{
	...
	static easyGetResourcesPath(_appRootPath){
	...
	static getMainExecutableCandidates(_appRootPath){
		switch(process.platform){
			case "darwin": // straightforward asf
				return [fs.readlink(path.resolve(_appRootPath, "Contents", "Frameworks", "Electron Framework.framework", "Electron Framework"))];
			case "win32": // almost straightforward
				_appRootPath = path.resolve(_appRootPath);
				let possibleFilenames = glob.sync(path.join(_appRootPath, "*.exe"));
			...
			case "linux":
				let possibleFilenames = glob.sync(path.join(_appRootPath, "!(*.*)"));
				possibleFilenames = possibleFilenames.filter(x => {
